...
from bsh_model_real2 import train
...
class ChromosomeDataset(Dataset):
    def __init__(self, tsv_dir,output_dir,checkpoint_file, bin_size=100000):
        ...
        self.completed_files = self.load_checkpoint()
        ...
        if os.path.exists(self.pkl_path):
            ...
            if info_df['Run'].is_unique:
                name_label_dict = dict(zip(info_df['Run'], info_df['platinumsensitivity']))
            else:
                print(
            ...
            # 遍历文件夹下所有数据文件，非excel文件，得到文件的样本编号，和文件对应的标签
            for tsv_file in tqdm(datafile_list):
                if 'xlsx' not in tsv_file:
                    ...
                    if checkkey in name_label_dict.keys():
                        ...
                        self.labels.append(1 if name_label_dict[checkkey] == 'Resistant' else 0)
    ...
    def save_result(self, result, tsv_file):
        output_file = os.path.join(self.output_dir, f"{os.path.basename(tsv_file)}.npy")
    ...
    def process_file(self, tsv_file):
        if 'xlsx' not in tsv_file:
            ...
            data = pd.read_csv(os.path.join(self.tsv_dir, tsv_file),
                               sep='\t', header=None,
                               names=['chromosome', 'position1','position2','weight'])
            try:
                out =   self.process_data(data,tsv_file)
            except Exception as e:
                print(f"Error occurred: {e}")          
            return out
        return None

    def load_result(self, tsv_file):
        output_file = os.path.join(self.output_dir, f"{os.path.basename(tsv_file)}.processed")
        with open(output_file, 'r') as f:
            return f.read()
    def merge_results(self):

        for file in os.listdir(self.output_dir):
            if file.endswith(".npy"):  # 确保只合并处理后的文件
                self.processed_data.append(np.load(os.path.join(self.output_dir, file)))
                # with open(os.path.join(self.output_dir, file), 'r') as f:
                #     self.processed_data.append(f.read())
        print(f"Results merged")

    def ordered_parallel_process_files(self, datafile_list):
        # 确定使用的CPU核心数
        num_workers = min(10, os.cpu_count()-3 or 1)  # 限制最大使用32个核心

        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            # 创建一个任务列表，但不立即开始执行
            futures = [
                executor.submit(self.process_file, tsv_file)
                for tsv_file in datafile_list if 'xlsx' not in tsv_file and tsv_file not in self.completed_files
                ]

            # 按照原始顺序处理结果
            # for future in tqdm(futures, total=len(futures), desc="Processing files"):
            for tsv_file, future in zip(datafile_list, tqdm(futures, total=len(futures), desc="Processing files")):
                result = future.result()  # 这会阻塞直到任务完成
                if result is not None:
                    self.save_result(result, tsv_file)
                    self.completed_files.add(tsv_file)  # 添加到完成列表
    ...
    def process_data(self, data,file_name):
        ...
        chrom2bin = {} # 染色体：该染色体分块数量长度的向量，向量中每一个元素表示该片段上有几个变异位点
        for key,value in chromosome_basepairs.items():
            max_position = value # 染色体长度
            # print(max_position)
            num_bins = max_position // self.bin_size + 1 # 染色体分块后的块数
            bins = np.zeros(num_bins, dtype=int)
            chrom2bin[key] = bins
        print(chrom2bin.keys())
        for _, row in data.iterrows():
            # print("test begin")
            bin_index1 = row['position1'] // self.bin_size # 判断该位置为哪一块染色体片段上
            bin_index2 = row['position2'] // self.bin_size # 判断该位置为哪一块染色体片段上
            chromosome = row['chromosome'][3:]
            # try:
            # print(row['chromosome'][3:], bin_index1,bin_index2, row['position'])
            if chromosome in chrom2bin.keys():
                error = bin_index2 + 1
                if bin_index2 + 1>len(chrom2bin[chromosome]):
                    error = len(chrom2bin[chromosome])
                    print('bin_index2 error:',bin_index2,error,file_name,row['chromosome'],row['position1'],row['position2'],row['weight'])
                for bin_now in range(bin_index1,error ):
                    begin = max(bin_now*self.bin_size,row['position1'])
                    end = min((bin_now+1)*self.bin_size,row['position2'])
                    chrom2bin[chromosome][bin_now] += (end-begin)*row['weight']
        ...
        return np.concatenate(chromosome_vectors) # 将一个样本的多条染色体对应的向量拼接在一起
...
def GetDNANames(bin_size=1000):
    ...
    for key, value in chromosome_basepairs.items():
        max_position = value # 染色体长度
        # print(max_position)
        num_bins = max_position // bin_size + 1 # 染色体分块后的块数
        start = 1
        for i in range(num_bins):
            # 染色体号：本块开头的编号_本块结尾的编号
...
if __name__ == "__main__":
    tsv_dir = '/mnt/data3/DNA/output' # dna文件都是tsv，标签文件是csv
    # dataloader = create_dataloader(tsv_dir)
    DNA_names = GetDNANames(bin_size=1000) # 根据染色体长度，得到分块后每块的起始位置和结束位置
    dataset = ChromosomeDataset(tsv_dir,'./output_data','checkpoint.json',bin_size=1000) # 处理整合样本数据
    ...
    # print(DNA_names)
    ID_train, ID_test, X_train, X_test, y_train, y_test = train_test_split(matched, Xdata, y_dna, 
