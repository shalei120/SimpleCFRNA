...
def train(X_train, y_train, X_valid, y_valid, nclass, feature_names, human_diseasename_list=None):
    ...
    regr = make_pipeline(
        #StandardScaler(),
        # SGDClassifier(loss='log_loss', penalty='elasticnet', alpha=0.000001, l1_ratio=0.15 ,random_state=42)
        #  LogisticRegression(C=0.00001,class_weight='balanced', penalty='l2',solver='saga',
        #                     tol=0.0001,random_state=42, verbose=1)
        # GradientBoostingClassifier(n_estimators=100, learning_rate=0.05, max_depth=5, 
        #                            random_state=0,verbose=1,subsample=1.0,max_features=None)
        # SGDClassifier(loss='log_loss',  alpha=0.0001, l1_ratio=0.15, random_state=42)
    # PCA(n_components=60),
        RandomForestClassifier(class_weight='balanced',n_estimators=100, min_samples_split= 2,
                              max_depth=5,random_state=42,min_samples_leaf= 2)###
    )
    regr.fit(X_train_gene, y_train)

    # regr=LogisticRegression(random_state=42, verbose=0)
    # param_grid = {
    #     'penalty': ['l2'],  
    #     'C': [0.001, 0.01, 0.1, 1],  
    ...
    # # # 创建 GridSearchCV 对象
    # regr = GridSearchCV(regr, param_grid, cv=3, scoring='roc_auc', verbose=1)
    ...
    for i in range(1, nclass):
        ...
        auc, best_thres = subdraw_roc_by_proba(ax, C_y_valid, C_y_probs,name=human_diseasename_list[i])
...
def subdraw_roc_by_proba(ax, y_valid, gbm_y_proba, name='', c='b'):
    ...
    print('gbm_threasholds',gbm_threasholds)
